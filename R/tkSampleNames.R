tkSampleNames <- function(...,filenames=character(0)){
  ...
  tkgrid(array.name.lst, row = 2, col = 1)
  ...
  tkgrid(desc.entry.lst, row = 2, col = 3)
  ...
  for(i in 1:NumSamples){
    ...
    tkgrid(array.name.lst[[i]], row = (i+2), col = 1)
    ...
    tkgrid(desc.entry.lst[[i]], row = (i+2), col = 3)
  ...
  tkgrid(finish.but, row = (NumSamples + 8), col = 2)
