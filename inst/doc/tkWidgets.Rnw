...
\begin{itemize}
\item \texttt{argsWidget} - an interface that allows users to input values for
    an argument list passed to the widget as an argument
\item \texttt{DPExplorer} - an interface that allows users to view a
    {\texttt{Bioconductor}} data package and make selections
\item \texttt{fileBrowser} - an interface that allows users to select a group of
    files from a local directory
\item \texttt{importWizard} - an interface that allows users to import a data file
    into R, following an Excel style
\item \texttt{objectBrowser} - an interface that allows users to browse R
    objects and make a selection
\item \texttt{vExplorer} - an interface that allows users to explore vignettes
...
not be of general interest. These widgets are thus not included in the examples. 
\end{document}
